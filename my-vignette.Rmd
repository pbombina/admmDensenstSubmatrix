...
Tne vizualization of the randomly generated matrix $\mathbf{G}$ helps us to understand its structure. It is clear that $\mathbf{G}$ contains a dense $50 \times 40$ block (in the bottom left corner).

![Visual representation of randomly generated $\mathbf{G}$](Rplot.jpeg){width=400px}

We can remove all noise and isolate an image of a rank-one matrix $X0$ with $mn$ nonzero entries.

![Visual representation of dense submatrix](Rplot01.jpeg){width=400px}


Then we vizualize matrix $Y0$ to see the number of disagreements between original matrix $G$ and $X0$.

![Disagreement between $\mathbf{G}$ and $\mathbf{X_0}$](Rplot02.jpeg){width=400px}
...
The following is a simple example on how one could use the package to analyze the collaboration network found in the JAZZ dataset. It is know that this network contains a clique of 100 nodes.  We have already prepared dataset to work with. More details can be found in the provided file `JAZZ_IN_R.R.`
